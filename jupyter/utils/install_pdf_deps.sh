...
cd /usr/local/texlive/bin/"$(uname -m)-linux"
./tlmgr install tcolorbox pdfcol adjustbox titling enumitem soul ucs collection-fontsrecommended

# pandoc installation
curl -L https://github.com/jgm/pandoc/releases/download/3.7.0.2/"pandoc-3.7.0.2-linux-${UNAME_TO_GOARCH[$(uname -m)]}.tar.gz"  -o /tmp/pandoc.tar.gz
